...
:root{
  ...
  --v2:#6366f1;
  --v3:#3730a3;
  --v-glow:rgba(79,70,229,0.14);
  --v-pale:rgba(79,70,229,0.07);
  --v-border:rgba(79,70,229,0.20);
  --green:#16a34a;
  --green-pale:rgba(22,163,74,0.07);
  --green-border:rgba(22,163,74,0.20);
  --red:#dc2626;
  --red-pale:rgba(220,38,38,0.07);
  --red-border:rgba(220,38,38,0.20);
  --sky:#0284c7;
  --sky-pale:rgba(2,132,199,0.07);
  --sky-border:rgba(2,132,199,0.20);
  --amber:#d97706;
...
.up{animation:fadeUp .38s cubic-bezier(.16,1,.3,1) forwards}
.spin-w{width:14px;height:14px;border-radius:50%;border:2px solid rgba(255,255,255,.2);border-top-color:#fff;animation:spin .7s linear infinite;flex-shrink:0}
...
.tabs-bar{
  ...
  background:var(--surface);padding:5px;border:1px solid var(--border);border-radius:14px;
...
.inp{
  ...
  color:var(--ink);padding:0 14px;font-size:15px;font-weight:500;
...
.sel-wrap select{
  width:100%;height:46px;border-radius:11px;
  border:1px solid var(--border2);background:var(--surface);
  color:var(--ink);padding:0 38px 0 14px;font-size:14px;font-weight:500;
  appearance:none;cursor:pointer;transition:border-color .15s;
}
.sel-wrap::after{
  content:'';position:absolute;right:12px;top:52%;
...
.fold-head{
  ...
  padding-bottom:12px;cursor:pointer;user-select:none;
}
.fold-icon{
  ...
  transition:background .12s,transform .22s cubic-bezier(.4,0,.2,1);
}
...
function PostCard({ post, index, followers }) {
  ...
  return (
    <div className="pcard">
      ...
      <p style={{ fontSize: 13.5, color: "var(--ink2)", lineHeight: 1.7, minHeight: 68, margin: 0, display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
        {cap}
      </p>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 20px 80px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 36, flexWrap: "wrap", gap: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 38, height: 38, borderRadius: 10, background: "var(--v)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            ...
            <div>
              <div style={{ fontSize: 17, fontWeight: 800, color: "var(--ink)", letterSpacing: "-.02em", lineHeight: 1.1 }}>Melange Digital</div>
              <div style={{ fontSize: 11, color: "var(--ink3)", fontFamily: "var(--mono)", letterSpacing: ".04em" }}>IG Analytics</div>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span className="pill" style={{ background: "var(--v-pale)", border: "1px solid var(--v-border)", color: "var(--v3)" }}>
        ...
        <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1fr 400px", gap: 28, marginBottom: 28, alignItems: "center" }}>

          {/* left: hero copy — strictly left-aligned */}
          <div>
            <h1 style={{ fontSize: "clamp(34px,4.5vw,58px)", fontWeight: 800, letterSpacing: "-.05em", lineHeight: 1.05, color: "var(--ink)", marginBottom: 14, textAlign: "left" }}>
              Instagram<br />
              <span style={{ color: "var(--v)" }}>analytics</span>
            </h1>
            <p style={{ fontSize: 16, color: "var(--ink2)", lineHeight: 1.8, fontWeight: 400, maxWidth: 460, textAlign: "left" }}>
              Pull real data from any public Instagram profile. Posts, engagement rates, collab detection — export clean reports instantly.
            </p>

            {/* quick stats — left-aligned, only after data loads */}
            {profile && (
              <div style={{ display: "flex", gap: 8, marginTop: 22, flexWrap: "wrap" }}>
                {[
                  { label: "Followers", value: fmt(followers) },
                  { label: "Avg ER", value: pct(A.erF) },
                  { label: "Posts analysed", value: fmt(A.n) },
                ].map(({ label, value }) => (
                  <div key={label} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "10px 16px", minWidth: 100 }}>
          ...
          <div className="card" style={{ padding: 24 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink)", letterSpacing: "-.01em" }}>Run a scrape</div>
                <div style={{ fontSize: 12.5, color: "var(--ink3)", marginTop: 3 }}>Public profiles · powered by Apify</div>
              </div>
              {loading && (
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 10, color: "var(--v)", fontFamily: "var(--mono)", fontWeight: 600 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 10, fontFamily: "var(--mono)", textTransform: "uppercase", letterSpacing: ".1em", color: "var(--ink3)", display: "block", marginBottom: 6, textAlign: "left" }}>Username</label>
              <input className="inp" type="text" placeholder="e.g. natgeo or @nasa" value={username} onChange={e => setUsername(e.target.value)} onKeyDown={e => e.key === "Enter" && !loading && startScrape()} />
            </div>
            ...
            <div className="g2" style={{ marginBottom: 16 }}>
              {[
                ...
                <div key={label}>
                  <label style={{ fontSize: 10, fontFamily: "var(--mono)", textTransform: "uppercase", letterSpacing: ".1em", color: "var(--ink3)", display: "block", marginBottom: 6, textAlign: "left" }}>{label}</label>
        ...
        {statusMsg && (
          <div className="status-bar up" style={{ background: stStyle.bg, color: stStyle.ink, borderColor: stStyle.border }}>
            ...
            {statusType === "success" && (
              <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2 7l3.5 3.5 5.5-6" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
            )}
            {statusMsg}
          </div>
        )}
                  ...
                  <div className="card" style={{ overflow: "hidden" }}>
                    <div style={{ display: "flex", gap: 18, alignItems: "flex-start", padding: "24px 26px", borderBottom: "1px solid var(--border)" }}>
                      <div style={{ position: "relative", flexShrink: 0 }}>
                        <div style={{ width: 68, height: 68, borderRadius: 18, background: "var(--v)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, color: "#fff", letterSpacing: "-.02em" }}>
                          {initials}
                        </div>
                        {isVerified && (
                          <div style={{ position: "absolute", bottom: -3, right: -3, width: 18, height: 18, borderRadius: "50%", background: "#1d9bf0", border: "2px solid var(--bg)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, color: "#fff", fontWeight: 800 }}>✓</div>
                        )}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 22, fontWeight: 800, color: "var(--ink)", letterSpacing: "-.03em", lineHeight: 1.15 }}>{profileName}</div>
                ...
                <FoldSection title="Posts" badge={`${filteredPosts.length} shown`} defaultOpen>
                  <div className="filters-bar">
                    ...
                    {(showCollab || showVideo) && (
                      <button className="btn-sm" onClick={() => { setShowCollab(false); setShowVideo(false); }} style={{ marginLeft: "auto" }}>Clear</button>
                    )}
                  </div>
                ...
                <FoldSection title="Export data" defaultOpen>
                  <div className="g2" style={{ gap: 12, marginBottom: 14 }}>
                    <div className="card" style={{ padding: 24 }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 700, color: "var(--ink)", marginBottom: 8, letterSpacing: "-.01em" }}>Excel Workbook</div>
                      <div style={{ fontSize: 13.5, color: "var(--ink2)", lineHeight: 1.7, marginBottom: 20 }}>3 formatted sheets: Posts with all metrics, Profile Summary, and Insights. Perfect for client reports.</div>
                    ...
                    <div className="card" style={{ padding: 24 }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 700, color: "var(--ink)", marginBottom: 8, letterSpacing: "-.01em" }}>JSON Data</div>
                      <div style={{ fontSize: 13.5, color: "var(--ink2)", lineHeight: 1.7, marginBottom: 20 }}>Raw post array with all computed metrics. Ready for pipelines, databases, or further processing.</div>
                  ...
                  <div style={{ padding: "12px 14px", borderRadius: 10, background: "var(--surface2)", border: "1px solid var(--border)", fontSize: 13, color: "var(--ink2)", lineHeight: 1.8 }}>
                    <span style={{ color: "var(--ink)", fontWeight: 600 }}>Fields: </span>
